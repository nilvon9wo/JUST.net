...
public class CustomFunctionsTests
{
	...
	public void RegisteredCustomFunction()
	{
		...
		const string transformer = "{ \"ExcessFormsList\": { \"#loop($.ExcessQuoteAmendments)\": { \"AttributesList\": { \"#loop($.Attributes)\": { \"Value\":\"#aliascustomfunction(#currentvalueatpath($.Value), #currentvalueatpath($.CoverageDataType))\" }}}}}";
		...
		string actual = new JsonTransformer(context).Transform(transformer, input);
	...
	{
		const string input = "{ \"ExcessQuoteAmendments\": [{ \"Name\": \"test_name\", \"Attributes\": [{ \"CoverageDataType\": \"test_coverage\", \"Value\": \"test_value\"}, { }] }]";
		const string transformer = "{ \"ExcessFormsList\": { \"#loop($.ExcessQuoteAmendments)\": { \"AttributesList\": { \"#loop($.Attributes)\": { \"Value\":\"#ExternalMethods::ExternalMethods.ExternalClass::CheckNullParameters(#currentvalueatpath($.Value), #currentvalueatpath($.CoverageDataType))\" }}}}}";

		string actual = new JsonTransformer().Transform(transformer, input);
	...
	{
		const string input = "{ \"ExcessQuoteAmendments\": [{ \"Name\": \"test_name\", \"Attributes\": [{ \"CoverageDataType\": \"test_coverage\", \"Value\": \"test_value\"}, { }] }]";
		const string transformer = "{ \"ExcessFormsList\": { \"#loop($.ExcessQuoteAmendments)\": { \"AttributesList\": { \"#loop($.Attributes)\": { \"Value\":\"#JUST.UnitTests.CustomFunctionsTests::CheckNullParameters(#currentvalueatpath($.Value), #currentvalueatpath($.CoverageDataType))\" }}}}}";
